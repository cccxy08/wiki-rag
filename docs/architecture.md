...
LLM Wiki 质量高度依赖底层模型能力，Qwen2.5-7B 在复杂任务上有明显短板：

| 能力 | Qwen2.5-7B | GPT-4 级别 |
|------|:---:|:---:|
| 结构化提取 | 良好 | 优 |
| 生成 Markdown | 良好（偶有格式错误） | 优 |
| 交叉引用 ([[...]]) | 较弱（常遗漏） | 优 |
| 矛盾检测 | 较弱（误报多） | 良好 |
| 数值推理 | 较弱（需显式提示） | 优 |

应对策略：
- **可插拔 LLM Provider**：一行配置切换到更强模型
- **Prompt 迭代**：针对小模型持续优化 Prompt
- **RAG 兜底**：Wiki 质量不足时自动降级到 RAG
- **Prompt 分级**：小模型用简化 Prompt，大模型用完整 Prompt

### 部署限制

- 本地部署需 Ollama，台式机在线才可用
- 生产环境建议 OpenAI/智谱/MiniMax 等云端 API
- 数据权限 `access_level` 字段已预留，待接入 SSO 后启用

### 扩展性风险

- Chroma 单机性能 <10 万文档，超出需迁移 Milvus/Qdrant
- Markdown 文件数 >1000 时 index 命中率下降，需引入向量检索辅助
- Wiki 页面膨胀后需定期清理和合并

### 用户体验风险

- Wiki 内容质量初期可能不够准确
- 单次 Ingest 耗时 20-30 秒，大文件可能更慢
- 首次查询响应慢（冷启动 Embedding 加载）
...
### 反向链接 (Backlinks) — 优先级 P0

让 Wiki 页面之间自动建立"谁引用了我"关系。
...
当"编辑 XX 项目页面"时，如果 XX 项目页面包含 [[张三]]，则自动在张三页面追加反向链接。
反向链接的维护由 LLM 在 Ingest 时自动完成。

### 页面分类体系 — 优先级 P1

扩展 `type` 字段为 person / project / policy / department / vendor 等类型，
使 Agent 能更精准地检索，如"只查 type=person 或 type=project 的页面"。
...
根据 LLM 能力层级选择不同复杂度的 Prompt：
- 7B 小模型：简化 Prompt，减少指令数量，优先保证 Ingest 成功率
- GPT-4 级大模型：完整 Prompt，包含交叉引用、矛盾检测等高级 Ingest 能力